//plugin-template: imageGalleryBackboneCollection.js

//Create an empty Collection to hold all the posts.
var ImageGalleryCollection = Backbone.Collection.extend({ //Collection Class
  ...
  parse: function(response) {
    ...
    if(response.collection.length == 0) {
      log.push('Empty data returned by server when trying to retrieve Image Gallery Collection. Most likely due to a new DB.');
      return [global.imageGalleryModel];
    } else {
  ...
  initialize: function(options) {
    ...
    this.url = '/api/imagegallery/list',
    ...
    this.on('reset', function() {
      ...
      log.push('Finished retrieving Image Gallery Collection data from server.');
